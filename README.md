...
*   **InfinityStack (ISF):** Infinite virtual storage for blocks and ores, accessible by commands or a professional **GUI**.
*   **AutoBlock (ABF):** Automatically compacts ores and materials into blocks (e.g., `Diamonds` ➜ `Diamond Blocks`) upon collection.
...
*   `isg <amount|pack|all>`: Withdraws items from virtual storage using the **Slot ID**.
*   `addisf` / `addasf` / `addabf`: Adds the item in your hand to the respective filter.
*   `remisf` / `remasf` / `remabf [slotid]`: Removes a filter by held item or specific **Slot ID**.
...
*   To prevent **"Message not found"** errors after updating to v1.5, please delete your old `messages.yml` or add the new chest debug keys.
