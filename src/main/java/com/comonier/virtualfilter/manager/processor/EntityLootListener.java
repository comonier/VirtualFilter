...
public class EntityLootListener implements Listener {
    ...
    public void onPickup(EntityPickupItemEvent event) {
        ...
        if (engine.process(player, stack)) {
            entity.remove(); event.setCancelled(true); playTeleportSound(player);
        } else if (VirtualFilter.getInstance().getFilterRepo().hasFilter(uuid, "abf", mat)) {
    ...
    public void onFish(PlayerFishEvent event) {
        ...
        if (!VirtualFilter.getInstance().getSettingsRepo().isAutoLootEnabled(player.getUniqueId())) return;
